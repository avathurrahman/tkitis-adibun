...
- Durable webhook event ledger with duplicate protection for Midtrans and Doku
...
- `POST /api/webhooks/midtrans` — verifies signature, records a webhook event, deduplicates retries, updates payment + subscription status
- `POST /api/webhooks/doku` — verifies Doku notification, records a webhook event, deduplicates retries, updates payment + subscription status
...
Ten migration files exist under `supabase/migrations/`:
