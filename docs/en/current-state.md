...
- Payment callback fallback route at `/payment/callback`
- In-memory rate limiting on contact, waitlist, payment, and AI routes with standard response headers
...
- Automated tests via Vitest + Testing Library covering API routes, payment providers/webhooks, AI guards, MDX helpers, auth/client hooks, rate limiting, and key forms
...
- `POST /api/profile` — updates the authenticated user's profile data
...
- Public forms and paid/AI mutation routes enforce in-memory rate limits and return `X-RateLimit-*` headers
...
- Admin dashboard at `/admin` — payment stats, subscription counts, recent payments table (gated by `user_roles`)
...
- `npm run test` — runs 78 automated tests across server-side and client-side feature coverage

## Database Migrations Ready To Apply

Six migration files exist under `supabase/migrations/`:
...
1. Apply the six SQL migrations to your Supabase project
2. Add `SUPABASE_SERVICE_ROLE_KEY` to `.env.local` so webhooks, profile writes, and admin reporting can use server-only access
